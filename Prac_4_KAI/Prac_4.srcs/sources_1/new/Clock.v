...
module WallClock(
	...
	reg MButton = 0;   //output of sw0_deb, debounced output
	reg HButton = 0;   //output of sw1_deb, debounced output
	reg Mprev = 0;     //previous Mbutton state, to check rising edge
	reg Hprev = 0;     //previous Hbutton state, to check rising edge
	
	// Instantiate Debounce modules here
	Debounce sw0_deb (CLK100MWHZ, sw[0] , Mbutton);
	Debounce sw1_deb (CLK100MWHZ, sw[1], Hbutton);
	...
	always @(posedge CLK100MHZ) begin
	   ...
	   Mprev <= Mbutton;   //update previous states
	   Hprev <= Hbutton;
	end
