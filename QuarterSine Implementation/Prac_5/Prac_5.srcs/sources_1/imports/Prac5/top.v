...
module top(
    ...
    //Quarter-sine variables
    reg [1:0] quarter = 0;    //keep track of which quarter we are in
    reg [8:0] f_clk = 0;       //trigger to change address
    //0: 0-89 deg, 1: 90-179 deg, 2: 180-269  deg, 3: 270-359 deg
    ...
    always @(posedge CLK100MHZ) begin       
        ...
        if (clkdiv >= f_clk) begin
            clkdiv[12:0] <= 0;
            case (quarter)
                0: begin    //going from zero up to 90 (1)
                    PWM <= douta;
                    if (addra >= 63) begin
                        quarter <= quarter + 1;
                        addra <= addra - 1;
                    end else addra <= addra + 1;
                end
                1: begin    //from 90 down to 180 (0)
                    PWM <= douta;
                    if (addra <= 0) begin
                        quarter <= quarter + 1;
                        addra <= addra + 1;
                ...
                2: begin    //from 180 down to 270 (-1)
                    PWM <= ~douta;
                    if (addra >= 63) begin
                        quarter <= quarter + 1;
                        addra <= addra - 1;
                ...
                3: begin    //from 270 back up to 360 (0)
                    PWM <= ~douta;
                    if (addra <= 0) begin
                        quarter <= quarter + 1;
                        addra <= addra + 1;
